...
## perf когда PMC недоступен
Когда PMC выключены, например на гостевой системе в облачной среде, perf начинает использовать software interrupt как основу - hrtimer, он срабывает раз в какое то количество миллисекунд к примеру. Соответственно, если собирать стату стектрейсов на CPU, подобно profile:99Hz то он точно не всё захватит, более того, есть функции ядра которые не могут быть прерваны программными прерваниями - это например функции планировщика, спинлоки или аппаратные прерывания, поэтому стек трейс их не получится перехватить.
